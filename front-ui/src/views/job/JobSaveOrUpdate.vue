    ...
    <a-form :form="form" @submit="handleSubmit" style="max-width: 600px">
      ...
      <a-form-item label="字段映射关系">
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-button type="primary" ghost @click="autoMapping" style="width: 100%">
              <a-icon type="thunderbolt" /> 自动映射同名字段
            </a-button>
          </a-col>
        </a-row>
      </a-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      autoMapping () {
        if (!this.sourceFields || !this.targetFields) {
          this.$message.warning('请先选择来源表和目标表')
          return
        }
        const sourceFields = this.sourceFields
        const targetFields = this.targetFields
        const newMappings = []
        sourceFields.forEach(s => {
          const target = targetFields.find(t => t.name.toLowerCase() === s.name.toLowerCase())
          if (target) {
            // Check if already exists in mappings
            const exists = this.mappings.some(m => m.sourceField === s.name && m.targetField === target.name)
            if (!exists) {
              newMappings.push({ sourceField: s.name, targetField: target.name })
            }
          }
        })

        if (newMappings.length > 0) {
          // If only one empty mapping exists, replace it
          if (this.mappings.length === 1 && !this.mappings[0].sourceField && !this.mappings[0].targetField) {
            this.mappings = newMappings
          } else {
            this.mappings = [...this.mappings, ...newMappings]
          }
          this.$message.success(`自动匹配了 ${newMappings.length} 个字段`)
        } else {
          this.$message.info('未找到同名匹配字段')
        }
      }
    }
  }
