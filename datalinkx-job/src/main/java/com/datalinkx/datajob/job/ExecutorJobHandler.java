...
public class ExecutorJobHandler {
	...
	@Value("${flinkx.host-ip:}")
	private String flinkXHostIp;

	public String execute(String jobId, String reader, String writer, Map<String, Object> otherSetting)
			throws Exception {
	...
	public String generateJobJsonFile(String jobId, String reader, String writer, String setting) {
		if (flinkXHostIp != null && !flinkXHostIp.isEmpty()) {
			// Replace localhost/127.0.0.1 with the configured host IP (for WSL/Docker)
			reader = reader.replaceAll("jdbc:mysql://127\\.0\\.0\\.1", "jdbc:mysql://" + flinkXHostIp)
					.replaceAll("jdbc:mysql://localhost", "jdbc:mysql://" + flinkXHostIp);
			writer = writer.replaceAll("jdbc:mysql://127\\.0\\.0\\.1", "jdbc:mysql://" + flinkXHostIp)
					.replaceAll("jdbc:mysql://localhost", "jdbc:mysql://" + flinkXHostIp);

			// Add allowPublicKeyRetrieval=true for MySQL 8.0 caching_sha2_password auth
			if (!reader.contains("allowPublicKeyRetrieval")) {
				reader = reader.replaceAll("(jdbc:mysql://[^?]+\\?)", "$1allowPublicKeyRetrieval=true&");
			}
			if (!writer.contains("allowPublicKeyRetrieval")) {
				writer = writer.replaceAll("(jdbc:mysql://[^?]+\\?)", "$1allowPublicKeyRetrieval=true&");
			}
		}

		String jobJson = String.format("{\"job\":{\"content\":[{\"reader\":%s,\"writer\":%s}],\"setting\":%s}}", reader,
				writer, setting);
